...
RUN apk add --no-cache --no-progress git make typos libmagic py3-pip postgresql \
...
RUN set -x \
  && castellum --version 2>/dev/null

ARG BININFO_BUILD_DATE BININFO_COMMIT_HASH BININFO_VERSION
LABEL source_repository="https://github.com/sapcc/castellum" \
  org.opencontainers.image.url="https://github.com/sapcc/castellum" \
