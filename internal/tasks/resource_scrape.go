...
func (c *Context) processResourceScrape(ctx context.Context, tx *gsql.Tx, res db.Resource, labels prometheus.Labels) error {
	...
	// TODO: filter asset_type and normalise DB
	dbResources, err := db.ResourceStore.Select(ctx, tx, `SELECT * FROM resources WHERE scope_uuid = $1`, res.ScopeUUID).Collect()
	if err != nil {
		return err
	}
	...
	for _, resource := range dbResources {
		assetInfo, ok := manager.InfoForAssetType(resource.AssetType).Unpack()
		if !ok {
			continue
		}
		resources[assetInfo.AssetType] = core.ResourceInfo{
			ScopeUUID:  resource.ScopeUUID,
			AssetType:  resource.AssetType,
			ConfigJSON: resource.ConfigJSON,
		}
	}

	return c.processResourceScrapes(ctx, tx, dbResources, resources, labels)
}

func (c *Context) processResourceScrapes(ctx context.Context, tx *gsql.Tx, resources []db.Resource, resourceInfos map[db.AssetType]core.ResourceInfo, _ prometheus.Labels) error {
	for _, res := range resources {
		...
		startedAt := c.TimeNow()
		assetUUIDs, err := manager.ListAssets(ctx, res.ScopeUUID, resourceInfos)
		...
		for _, uuid := range assetUUIDs[res.AssetType] {
		...
		for _, assetUUID := range assetUUIDs[res.AssetType] {
