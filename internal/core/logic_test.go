...
func TestGetEligibleOperations(t *testing.T) {
	...
	//
	// Entirely conflicting constraints of equal priority (max < min or smax < smin) shall paralyze Castellum and
	...
	check(
		"low=20%, high=80%, crit=95%, step=20%, max=1050, min_free=200",
		"size=1000, usage=900",
		"high->1200", "high->1126", // MinimumFreeSize takes precedence over maximum constraint
	)
	check(
		"low=20%, high=80%, crit=95%, step=20%",
