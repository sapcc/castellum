...
func TestGetPendingOperationsForResource(t *testing.T) {
	...
	must.SucceedT(t, db.PendingOperationStore.Update(ctx, s.DB, pendingOp))
...
func TestGetRecentlyFailedOperationsForResource(t *testing.T) {
	withEitherFailedOrErroredOperation(func(failedOperationOutcome castellum.OperationOutcome) {
		...
		// not yet finished
		must.SucceedT(t, db.PendingOperationStore.Insert(ctx, s.DB, &db.PendingOperation{
			AssetID:   1,
			Reason:    castellum.OperationReasonHigh,
			OldSize:   1024,
			NewSize:   2048,
		...
		must.SucceedT(t, db.FinishedOperationStore.Insert(ctx, s.DB, &db.FinishedOperation{
			AssetID:     1,
			Reason:      castellum.OperationReasonHigh,
